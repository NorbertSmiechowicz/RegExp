...
##3. Extended Regular Expressions
    ...
    05. ere_duplication_symbol              :=  <asterisk>
                                             ...
                                             |  <open_brace>  ERE_DUPLICATION_COUNT  <comma>                          <close_brace>
                                             |  <open_brace>  ERE_DUPLICATION_COUNT  <comma>   ERE_DUPLICATION_COUNT  <close_brace>
...
#define REGEXP_TERM_TOKEN_LIST\
    REGEXP_TERM_TOKEN_ELEM( Backslash, "\\") /* Haha ha... */\
    REGEXP_TERM_TOKEN_ELEM( Period, ".")\
    REGEXP_TERM_TOKEN_ELEM( Comma, ",")\
    REGEXP_TERM_TOKEN_ELEM( Asterisk, "*")\
    REGEXP_TERM_TOKEN_ELEM( QuestionMark, "?")\
    REGEXP_TERM_TOKEN_ELEM( Plus, "+")\
    REGEXP_TERM_TOKEN_ELEM( Pipe, "|")\
    REGEXP_TERM_TOKEN_ELEM( Hyphen, "-")\
    REGEXP_TERM_TOKEN_ELEM( Circumflex, "^")\
    REGEXP_TERM_TOKEN_ELEM( Dollar, "$")\
    REGEXP_TERM_TOKEN_ELEM( OpenParenthesis, "(")\
    REGEXP_TERM_TOKEN_ELEM( CloseParenthesis, ")")\
    REGEXP_TERM_TOKEN_ELEM( OpenBrace, "{")\
    REGEXP_TERM_TOKEN_ELEM( CloseBrace, "}")\
    REGEXP_TERM_TOKEN_ELEM( OpenBracket, "[")\
    REGEXP_TERM_TOKEN_ELEM( CloseBracket, "]")\
    REGEXP_TERM_TOKEN_ELEM( OpenDot, "[.")\
    REGEXP_TERM_TOKEN_ELEM( DotClose, ".]")\
    REGEXP_TERM_TOKEN_ELEM( OpenEqual, "[=")\
    REGEXP_TERM_TOKEN_ELEM( EqualClose, "=]")\
    REGEXP_TERM_TOKEN_ELEM( OpenColon, "[:")\
    REGEXP_TERM_TOKEN_ELEM( ColonClose, ":]")\

#define REGEXP_TERM_TOKEN_ELEM( ID, TEXT) ID,

enum class RegExTerminalTokenId : std::size_t
{
    REGEXP_TERM_TOKEN_LIST
    Count
};

#undef REGEXP_TERM_TOKEN_ELEM
#define REGEXP_TERM_TOKEN_ELEM( ID, TEXT) TEXT,

static constexpr std::array< std::string_view, static_cast< std::size_t>( RegExTerminalTokenId::Count)> m_TerminalTokenArray
{
    REGEXP_TERM_TOKEN_LIST
};

#undef REGEXP_TERM_TOKEN_ELEM
#undef REGEXP_TERM_TOKEN_LIST

////////////////////////////////////////////////

enum class RegExLexicalTokenId
{
};

struct RegExLexicalToken
{
    RegExLexicalTokenId         m_Id;
...
public:
    ...
    template< RegExTerminalTokenId tokenId>
    bool
    try_token()
    {
        if constexpr( tokenId < RegExTerminalTokenId::Count)
        {
            std::string_view token = m_TerminalTokenArray[ static_cast< std::size_t>( tokenId)];
